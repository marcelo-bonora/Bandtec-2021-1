public class ListaEstatica {
    ...
    // Método p/ substituir
    public boolean substituirElemento(int valorAntigo, int valorNovo){
    ...
    // Métodos contar ocorrencias
    public int contarOcorrencias(int valorOcorrencia){
        int count = 0;
        for (int i = 0; i < nroElem; i++){
            if (vetor[i] == valorOcorrencia){
                count++;
            }
        }
        System.out.println("Quantidade de vezes que o valor " +valorOcorrencia+ " aparece : " +count);
        return count;
    }

    // Método adiciona no inicio
    public boolean adicionaNoInicio(int valorInicio){
